---
title: "SPREAD: Spatio-temporally Precise Rodent Community Ecology And Disease Database."
...
The database would serve as a convenient data-hosting service, and clear labelling of data points with the publication of origin and authors would promote collaboration and opportunities for future projects. The full dataset would be available for anyone to use in their research, making the most out of the resources and effort put in by scientists in the field. The SPREAD database will serve as a community-driven tool to be continually added to, ever improving in completeness and utility.


## Requirements for inclusion

![Example of included data, individual level data (one row per rodent) is also accepted. This table is to demonstrate resolution, **data do not have to exactly match this format**.](../figures/data_example.png)

The minimum requirements for data to be included in the SPREAD database are as follows:

1. **Trapping session dates** (minimum resolution of a month)
1. **Duration of trapping** (or start and end date)
1. **Precise coordinates for all trapping locations** (if sites not recorded, indication of trapping area in kmsq)
1. **All captured species' abundances** are reported (separately for each trapping session and site)
1. **All individuals are tested** for pathogen(s) of interest.
1. **Diagnostic assay and pathogen name** (at least family level e.g. hantavirus)
...
If you do not have access to the data but know someone who does, please pass this email/document along. For any questions, please email **harry.gordon@nhm.ac.uk**

## The SPREAD database as of Sept. 2024

```{r plot_n_hosts, echo=FALSE, fig.pos="h", message=FALSE, error=FALSE, warning=FALSE, fig.cap="The number of hosts trapped for each genus", fig.align='center'}
# Load necessary libraries
library(ggplot2)
library(dplyr)
library(scico)
...
```{r plot_n_infections, fig.pos="!", echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.cap="The number of infections for each virus tested", fig.align='center'}
# Load necessary libraries
library(ggplot2)
library(dplyr)
library(scico)
