...
# Create aggregated list of host and pathogens without uniq_ids and summing tested and positive for rows with same virus and species
# Read the CSV file (assuming your file is in the current working directory)
file_path <- "~/Desktop/pathogen_host list.xlsx"
...
# Ensure the numeric columns are indeed numeric
data2 <- cleaned_data2 %>%
  mutate(across(all_of(numeric_columns), as.numeric))

# Aggregate the dataset by summing numeric columns for each group of word columns
